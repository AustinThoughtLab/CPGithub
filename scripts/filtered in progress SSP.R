...
df <- read.csv("data/300 for R 3.22.22.csv")

View(df)

df2 <- df %>% 
  select(-p_languages, -p_duration, -X, -X.1, -X.2, -X.3, -X.4, -X.5, -X.6, 
         -X.7, -X.8) #dont think it deletes those end columns that are numbers 1-8 /useless, later renamed them na1/na2 etc can delete after that?
