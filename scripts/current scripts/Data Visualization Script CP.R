...
cinjplot <- cinjurysum %>%
  mutate(injury = fct_relevel(cinjurysum$injury, "stomach.ache", "bruised.leg", "burned.tongue",
                              "paper.cut", "splinter", "skinned.knees", "bee.sting", 
...
# facet_wrapped by age and injury 
## need to figure out how to reorder all three by injury sum

cageinjsum <- cp %>%
  group_by(cp_age, injury) %>%
  summarise(
    counts = n(),
    means = mean(rating, digits = 5),
    sds = sd(rating)) %>%
...
cageinjsum <- cageinjsum %>%
  mutate(cp_age= case_when(cp_age == "4" ~ "4 (n=38)",
                          cp_age == "5" ~ "5 (n=38)",
                          cp_age == "6" ~ "6 (n=13)"))


cageinjplot <- cageinjsum %>%
  ggplot(aes(x=reorder(injury, means), y=means)) +
  geom_col(aes(fill= factor(injury))) +
  facet_wrap(~cp_age) +
  geom_errorbar(aes(ymin = means - cis, ymax = means + cis), width = 0.3) +
  labs(x="Injury", y="Mean Pain Rating (0-7)") +
...
exyn <- exyn %>%
  mutate(val = (ifelse(exyn$value=="yes",1,0)))
...
# exclusions ratings of pain vvvv this doesn't work!!!!!
##  exclusions does not have it sorted by injury, need to resort
...
cexcplot <- cexcsum %>%
  mutate(injury = fct_relevel(cexcsum$injury, "stomach.ache", "bruised.leg", "burned.tongue",
                              "paper.cut", "skinned.knees", "splinter", "bee.sting", 
                              "broken.arm")) %>% #manually ordered by ascending mean; 
  #can just do ggplot(aes(x=reorder(injury, means), y=means) but not in rainbow order
  ggplot(aes(x=injury, y=means)) +
...
## Plot of mean pain rating by injury with 95% confidence intervals
...
# for ordering means if there are two identical means when rounded to 2 digits
# -> only if keeping rainbow order from above
rmeans <- round(x = cinjurysum$means, digits = 5)
